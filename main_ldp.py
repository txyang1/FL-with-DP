...
if __name__ == '__main__':
    ...
    for iter in range(args.epochs):  # 循环进行多个训练轮次 default=10 t= 1,2,3....
        ...
        C = 60  # Gradient norm bound
        sigma = 0.01 # Noise scale

        # Clipping and adding noise
        clipped_grad = []
        for grad in grad_locals:
            grad_norm = 0.0
            # Iterate over each tensor in the state_dict
            for key, param in grad.items():
                if param is not None:  # Ensure the tensor is not None
            ...
            # Clip the gradient
            clipped = {name: torch.zeros_like(param) for name, param in grad.items()}
            if grad_norm > C:
                for key, param in grad.items():
            ...
            clipped_grad.append(clipped)

        # Add noise to the clipped gradients
        noisy_grad = []
        for grad in clipped_grad:
            noisy = {name: param + sigma*C * torch.randn_like(param) for name, param in grad.items()}
            noisy_grad.append(noisy)
        #average the noisy_grad
        avg_grad = Avg(noisy_grad)

        # Now clipped_grad contains the clipped and noised gradients
        ...
        if args.Agg == 'Krum':
           ...
           #grad_glob, min_index = Krum(clipped_grad, args.malicious)
           grad_glob, min_index = Krum(noisy_grad, args.malicious)
        else:
            grad_glob = Avg(noisy_grad)
    ...
    with open('./result/LDP_{}_{}_attack_{}_epsilon_{}_addtime_{}_malicious_{}_epoch_{}_users_{}.txt'.format(args.dataset,args.Agg, args.attack,args.epsilon,args.addtime,args.malicious,args.epochs,args.frac), 'w') as f:
