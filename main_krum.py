...
if __name__ == '__main__':
    ...
    cv_loss, cv_acc = [], []#用于交叉验证的损失和精度（可能在后续代码中使用）。
    val_loss_pre, counter = 0, 0#用于早停法或其他用途的变量
    net_best = None#用于保存最佳模型
    best_loss = None#保存最佳损失值
    val_acc_list, net_list = [], []#用于保存验证精度和网络列表（可能在后续代码中使用）。
    ...
    for iter in range(args.epochs):#循环进行多个训练轮次 default=10 t= 1,2,3....
        ...
        m = max(int(args.frac * args.num_users), 1)#计算参与训练的客户端数量，确保至少有一个客户端。
        idxs_users = np.random.choice(range(args.num_users), m, replace=False)#随机选择参与训练的客户端索引。
        for idx in idxs_users:#：遍历选定的客户端 k k=10
        ...
        #Fedaverage
        #w_glob_fedave = FedAvge(w_locals)
        #krum
        f=11#n>2f+2 n=10 --> f<4
        #print(len(w_locals),f)
        w_glob,min_index = Krum(w_locals,f)
        ...
        #{:3d} 表示整数格式，宽度为3。
        #{:.3f} 表示浮点数格式，保留三位小数。
        print('Round {:3d}, Average loss of krum {:.3f}'.format(iter, loss_avg))
